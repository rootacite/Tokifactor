...
/**
 * On-wire file fragment: same shape as [PictureChunk] plus the original `name`.
 *
 * `id` is `{transferId}:{index}:{total}`.
 * `hash` is SHA-256 hex of the full plaintext file (same on every chunk).
 * `encrypted_data` is Base64 of one slice of the full-file ciphertext.
 */
data class FileChunk(
    ...
    companion object {
        ...
        fun decode(payload: String): FileChunk? {
            ...
            val name = sanitizeFileName(WireJson.extractString(payload, "name") ?: "")
            val parts = id.split(':')
        ...
        fun sanitizeFileName(raw: String): String {
            ...
            return leaf.ifBlank { "file.bin" }.take(200)
        }
    }
}
